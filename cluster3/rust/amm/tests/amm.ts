...
describe('amm-2023', () => {
  ...
  const programId = new PublicKey(
    'GfHrF6mrJEPCu57UGK34d8Jht4ntm86siVqHNGo7SYNA'
  );
  ...
  it('Create mints, tokens and ATAs', async () => {
    ...
    user_x_ata = await getAssociatedTokenAddress(
      mint_x,
      user.publicKey,
      false,
      tokenProgram
    );
    user_y_ata = await getAssociatedTokenAddress(
      mint_y,
      user.publicKey,
      false,
      tokenProgram
  ...
  it('Initialize', async () => {
    try {
      const tx = await program.methods
        .initialize(seed, 1, initializer.publicKey)
        .accounts({
          auth,
